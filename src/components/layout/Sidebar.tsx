    ...
    <aside className="relative flex w-52 flex-shrink-0 flex-col bg-sidebar">
      ...
      <div className="flex items-center gap-2.5 px-4 pb-3 pt-4">
        <div className="flex h-[26px] w-[26px] flex-shrink-0 items-center justify-center rounded-[7px] bg-white/[0.09]">
          <span className="select-none font-bold text-[11px] leading-none text-white/60">F</span>
        </div>
        <span className="select-none font-semibold text-[13px] tracking-[-0.02em] text-white/60">
          FialhoClean
        </span>
      </div>

      {/* ── Section label ── */}
      <p className="px-4 pb-1 pt-1 text-[10px] font-semibold uppercase tracking-[0.07em] text-white/20">
        {t("nav.label")}
      </p>
        ...
        {NAV_ITEMS.map(({ id, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`group flex w-full items-center gap-2.5 rounded-lg px-2.5 py-[7px] text-left transition-colors duration-150 focus-ring ${
                isActive
